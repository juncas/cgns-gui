...
- **第 7 周**：M8.5（新增 - 高优先级）
  - 将 CGNS 数据加载从 h5py 完全迁移到 pyCGNS。
  - 重构 CgnsLoader 使用 CGNS.MAP 和 CGNS.PAT 模块。
  - 保留完整的 CGNS/SIDS 树结构，避免数据丢失。
  - 为后续流场数据支持打下坚实基础。
- **第 8 周**：M9
  - 扩展 CGNS 解析以加载流场等数据集。
...
- [x] 固定坐标显示组件（轴向指示），无论窗口大小或场景变化均保持可见。
- [x] 为上述改动补充交互层单测或演示脚本，并更新文档。
- [x] Surface 模式网格线隐藏，高亮时显示边缘，Wireframe 模式无边缘线。
- [x] 禁用 VTK 输出窗口弹窗，避免干扰用户体验。

### M8.5：迁移到 pyCGNS（新增 - 高优先级）
- [ ] 调研 pyCGNS 架构，理解 CGNS.MAP 和 CGNS.PAT 模块的使用方式。
- [ ] 添加 pyCGNS 依赖到 `pyproject.toml`，确保在各平台可正常安装。
- [ ] 重构 `CgnsLoader` 类，使用 `CGNS.MAP.load()` 替代 h5py 直接读取。
- [ ] 保留完整的 CGNS/SIDS 树结构信息（Family、ZoneBC、GridLocation 等）。
- [ ] 更新 `model.py` 数据模型，适配 CGNS/Python 树的节点访问方式。
- [ ] 重写所有 `test_loader.py` 单元测试，验证网格、边界条件等数据正确解析。
- [ ] 测试现有 CGNS 文件，确保可视化功能不受影响。
- [ ] 编写迁移文档，记录 API 变化和注意事项。
...
| T-015 | 坐标显示固定化 | M8 | ⭐⭐ | 已完成 | - |
| T-016 | Surface 模式线框策略调整 | M8 | ⭐⭐ | 已完成 | - |
| T-017 | VTK 输出窗口禁用 | M8 | ⭐ | 已完成 | - |
| T-018 | pyCGNS 架构调研与依赖配置 | M8.5 | ⭐⭐⭐ | 未开始 | - |
| T-019 | 重构 CgnsLoader 使用 pyCGNS | M8.5 | ⭐⭐⭐ | 未开始 | - |
| T-020 | 更新数据模型适配 CGNS 树结构 | M8.5 | ⭐⭐⭐ | 未开始 | - |
| T-021 | 重写加载器单元测试 | M8.5 | ⭐⭐⭐ | 未开始 | - |
| T-022 | 验证迁移后可视化功能完整性 | M8.5 | ⭐⭐⭐ | 未开始 | - |
| T-023 | 流场数据解析扩展 | M9 | ⭐⭐⭐ | 未开始 | - |
| T-024 | 云图/切片可视化实现 | M9 | ⭐⭐⭐ | 未开始 | - |
| T-025 | 跨平台回归测试与文档更新 | M10 | ⭐⭐⭐ | 未开始 | - |
